...
function convertZodObject(schema, exportedSchemas) {
  return {
    type: "object",
    fields: Object.entries(schema._def.shape()).filter((pair) => pair[1] instanceof ZodType).map(([key, value]) => ({
      key,
...
function findZodSchemas(modules) {
  return Object.entries(modules).flatMap(([path, mod]) => {
    if (mod instanceof ZodType2) {
      return [{ schema: mod, path }];
    }
    return Object.entries(mod).filter(
      (pair) => pair[1] instanceof ZodType2
    ).map(
      ([name2, schema]) => ({
...
async function parseArgs(argv) {
  const program = new Command().name(name).description(description).version(version).option("-c, --config <path>").option("-e, --entry <paths...>").option("-o, --output <path>").option("-t, --title <text>").option("--tsconfig <path>").addOption(
    new Option("-f, --format <format>").choices(["esm", "cjs"])
  );
...
async function runCLI(handler, argv) {
  ...
  await mkdir(dirname2(config.output), { recursive: true });
